...
import { setCourtId, setPlayerId } from '../../../reduxfolder/courtSlice';
...
const ShaduleRevisa = ({navigation}) => {
  ...
  const sendMessage = () => {
    ...
    dispatch({
      type: 'COURTId',
      payload: { courtId: ReciverId }
    });
     navigation.navigate('chatscreen', { userName: CourtName, userImg: CourtImage, messageTime: '' })
    
  };

  // console.log('CourtId',receiverId,'senderId',senderId)
  useEffect(() => {
...
const renderItem = ({ item }) => (
  <TouchableOpacity onPress={() => {
    ...
    setCourtImage(item.courtImage)
    setReciverID(item.courtOwnerId)
    // console.log('item.courtOwnerId',item.courtOwnerId)
    
    // dispatch({
    //   type: 'COURTId',
    //   payload: { courtId: item.courtOwnerId }
    // });
    
    // dispatch({
    //   type: 'PLAYERId',
    //   payload: { playerId: item.bookedBy }
    // });
  }}>
                ...
                <View style={styles.modalContainer}>
                    <View style={styles.modalContent}>
                      ...
                      <View style={[styles.DisplayFlexProp,{width:"100%",paddingTop:10,borderBottomColor:'rgba(33, 33, 33, 0.25)',borderBottomWidth:0.5,paddingBottom:5}]}>
                        ...
                        <Button text="mensaje"  Link={sendMessage} />
                        </View>
                    </View>
                </View>
            </Modal>
