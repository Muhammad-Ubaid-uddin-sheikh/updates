...
// const API_URL = 'https://kickers-backend-5e360941484b.herokuapp.com/api/court/getMyChatRooms';
// const Messages = [
//     {
//       id: '1',
//       userName: 'Jenny Doe',
...
//         navigation.navigate('CourtChatScreen',{data: chatRoomId, userName: name, userImg:'', messageTime: "nomessage" })
//       };
//       // const handleChatRoomClick = (chatRoomId) => {
//       //   const socket = io(SocketUrl);
//       //   socket.emit('join', { chatRoomId:chatRoomId});
//       //   // navigation.navigate('chatscreen', { userName: item.userName, userImg: item.userImg, messageTime: item.messageTime })
//       //   navigation.navigate('CourtChatScreen', { userName: 'Hello', userImg:'', messageTime: "nomessage" })
//       // };
//     return (
...
//             renderItem={({ item }) => (
//               <TouchableOpacity onPress={() =>handleChatRoomClick(item._id,item.name)}>
//                 <View style={styles.userInfo}>
//                   <View >
//                     <Image style={{ width: 45, height: 45, borderRadius: 8 }} source={{ uri: item.userImg }}  />
//                   </View>
...
const Chat = ({ navigation }) => {
    ...
    const [ChatroomData,SetChatroomData] = useState()
    const socket = useSocket();
    console.log('ChatroomId', ChatroomData)
    useEffect(() => {
        const fetchDataAndStore = async () => {
          try {
            const token = await AsyncStorage.getItem('accessTokenCourt');
            ...
            if (token) {
              ...
              if (response.ok) {
                ...
                const finalData = data.data
                console.log('datanewasd',finalData)
                console.log("chatroom data",data.data)
                // const extractedIds = finalData.map(item => item._id);
                SetChatroomData(finalData)
      ...
      const handleChatRoomClick = (chatRoomId,name) => {
        ...
        navigation.navigate('CourtChatScreen',{data: chatRoomId, userName: name, userImg:'https://w0.peakpx.com/wallpaper/454/383/HD-wallpaper-football-super-wonderful-nice-awesone-ground-stadium.jpg', messageTime: "nomessage" })
      };
    return (
        <View style={styles.container}>
        {loading ? (
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity onPress={() =>handleChatRoomClick(item.chatRoomId,item.name)}>
                <View style={styles.userInfo}>
                  <View >
                    <Image style={{ width: 45, height: 45, borderRadius: 8 }} source={{ uri: 'https://w0.peakpx.com/wallpaper/454/383/HD-wallpaper-football-super-wonderful-nice-awesone-ground-stadium.jpg' }}  />
                  </View>
                  <View style={styles.textSec}>
                    <View style={styles.textInfoView}>
                      <Text style={styles.textNmae}>{item.name}</Text>
